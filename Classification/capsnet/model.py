...
class Model(ModelBase):
    ...
    def __init__(self, model_name, output_folder, nb_label):
        """
            **input:
                *model_name: (Integer) Name of this model
                *output_folder: Output folder to saved data (tensorboard, checkpoints)
        """
        ModelBase.__init__(self, model_name, output_folder=output_folder, nb_label=nb_label)
    ...
    def _build_decoder(self, caps2, one_hot_labels, batch_size):
        """
            ...
            **input:
                ...
                *batch_size
        """
        labels = tf.reshape(one_hot_labels, (-1, self.nb_label, 1))
        # squeeze(caps2):   [?, len_v_j,    capsules_nb]
        # labels:           [?, nb_label,  1] with capsules_nb == nb_label
    ...
    def init(self):
        ...
        one_hot_labels = tf.one_hot(self.tf_labels, depth=self.nb_label)
    ...
    def _build_loss(self, caps2, one_hot_labels, labels, decoded, images):
        ...
        max_l = tf.reshape(max_l, shape=(-1, self.nb_label))
        max_r = tf.square(tf.maximum(0., capsules_length - 0.1))
        max_r = tf.reshape(max_r, shape=(-1, self.nb_label))
        t_c = one_hot_labels
    ...
    def predict(self, images):
        ...
        caps2 = np.reshape(caps2, (len(images), self.nb_label))
        # softmax
...
if __name__ == '__main__':
    model = Model("test", output_folder=None)
    model.init()
